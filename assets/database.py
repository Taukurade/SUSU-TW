...
Список доступных команд:
    ...
    def get_catalog():
        table = [['id', 'ISBN', 'Авторы', 'Дата публикации',
                  'Название', 'Текущий владелец']]
        for book in Catalog.select():
            try:
                data = requests.get(
                    f"https://www.googleapis.com/books/v1/volumes?q=isbn:{book.ISBN}").json()['items'][0]['volumeInfo']
            except KeyError:
                continue
            table.append([book.id, book.ISBN, ", ".join(
                data['authors']), data['publishedDate'], data['title'], book.Using_by])
